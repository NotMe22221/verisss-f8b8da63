...
import { onInView } from "@/lib/anime";
...
export function PrototypeSection() {
  ...
  useEffect(() => {
    ...
    if (!el) return;
    return onInView(el, () => el.classList.add("in"));
  }, []);
  ...
  return (
    <section id="prototype" className="bg-[#1B3A4B] px-6 lg:px-12 py-24 lg:py-36">
      <div className="max-w-[88rem] mx-auto">
        ...
        <div ref={railRef} className="relative timeline-rail reveal-stagger">
          {/* Drawn rail */}
          <div aria-hidden className="hidden md:block absolute left-0 right-0 top-8 h-px overflow-hidden">
            <div className="rail-line h-full w-full origin-left bg-gradient-to-r from-transparent via-[#C9A46A]/45 to-transparent" />
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-[#F4EFE6]/10 rounded-2xl overflow-hidden relative">
            {milestones.map((m, i) => (
              <div key={m.tag} className="bg-[#1B3A4B] p-6 lg:p-8 min-h-44 flex flex-col justify-between relative">
                <span
                  aria-hidden
                  className="rail-dot hidden md:block absolute -top-1 left-8 w-2.5 h-2.5 rounded-full bg-[#C9A46A]"
                  style={{ boxShadow: "0 0 0 4px rgba(201,164,106,0.18)", transitionDelay: `${500 + i * 200}ms` }}
                />
          ...
          <style>{`
            .timeline-rail .rail-line { transform: scaleX(0); transition: transform 1500ms cubic-bezier(0.2,0.7,0.2,1) 200ms; }
            .timeline-rail.in .rail-line { transform: scaleX(1); }
            .timeline-rail .rail-dot { transform: scale(0); opacity: 0; transition: transform 600ms cubic-bezier(0.2,0.7,0.2,1), opacity 600ms ease; }
            .timeline-rail.in .rail-dot { transform: scale(1); opacity: 1; }
          `}</style>
        </div>
      </div>
    </section>
